...
from drf_spectacular.plumbing import (
    ...
    append_meta,
    assert_basic_serializer,
    build_array_type,
    build_basic_type,
    build_choice_field,
    build_examples_list,
    build_generic_type,
    build_listed_example_value,
...
class AutoSchema(ViewInspector):
        ...
        for parameter in self.get_override_parameters():
            if isinstance(parameter, OpenApiParameter):
                ...
                if is_basic_type(parameter.type):
                    ...
                    warn(
                        f'unsupported type for parameter "{parameter.name}". Skipping.'
                    )
                    continue
                ...
                for property_name, property_schema in mapped["properties"].items():
                    ...
                    result[
                        property_name, OpenApiParameter.QUERY
                    ] = build_parameter_type(
                        name=property_name,
    ...
    def get_auth(self):
        ...
        for authenticator in self.view.get_authenticators():
            if not whitelisted(
                authenticator, spectacular_settings.AUTHENTICATION_WHITELIST, True
            ):
                continue
            ...
            if isinstance(scheme.name, str):
                names, definitions = [scheme.name], [
                    scheme.get_security_definition(self)
                ]
            else:
        ...
        if permissions.AllowAny in perms:
            ...
            permissions.IsAuthenticatedOrReadOnly in perms
            and self.method in permissions.SAFE_METHODS
        ):
            auths.append({})
        return auths
    ...
    def _tokenize_path(self):
        # remove path prefix
        path = re.sub(
            pattern=self.path_prefix, repl="", string=self.path, flags=re.IGNORECASE
        )
    ...
    def _map_model_field(self, model_field, direction):
        ...
        if field and isinstance(field, serializers.PrimaryKeyRelatedField):
            ...
            if not field.child_relation.queryset:
                field.child_relation.queryset = get_manager(
                    model_field.related_model
                ).none()
            return self._map_serializer_field(field, direction)
        elif field and not isinstance(
    ...
    def _map_serializer_field(self, field, direction, bypass_extensions=False):
        ...
        if isinstance(
            ...
            if getattr(field, "queryset", None) is not None:
                if is_slug:
                    ...
                    model_field = follow_field_source(
                        model, source, default=models.TextField()
                    )
                else:
            ...
            if callable(model_field):
                ...
                assert (
                    False
                ), f'Field "{field.field_name}" must point to either a property or a model field.'
        ...
        if isinstance(field, serializers.ListField):
            if isinstance(field.child, _UnvalidatedField):
                return append_meta(
                    build_array_type(build_basic_type(OpenApiTypes.ANY)), meta
                )
        ...
        if isinstance(field, serializers.DecimalField):
            if getattr(
                ...
                if field.max_whole_digits:
                    content["pattern"] = (
                        rf"^-?\d{{0,{field.max_whole_digits}}}"
                        rf"(?:\.\d{{0,{field.decimal_places}}})?$"
                    )
            else:
        ...
        if isinstance(field, serializers.IntegerField):
            ...
            if not all(
                -2147483648 <= int(content.get(key, 0)) <= 2147483647
        ...
        if isinstance(field, (serializers.DictField, serializers.HStoreField)):
            ...
            if not isinstance(field.child, _UnvalidatedField):
                ...
                self._insert_field_validators(
                    field.child, content["additionalProperties"]
                )
            return append_meta(content, meta)
        ...
        if isinstance(field, serializers.ReadOnlyField):
            ...
            if is_list_serializer(field.parent):
                model = getattr(
                    getattr(field.parent.parent, "Meta", None), "model", None
                )
    ...
    def _get_serializer_field_meta(self, field, direction):
        ...
        if field.label and not is_trivial_string_variation(
    ...
    def _map_basic_serializer(self, serializer, direction):
        ...
        for field in serializer.fields.values():
            ...
            add_to_required = field.required or (
                schema.get("readOnly")
            ...
            if schema_type == "string":
                if isinstance(v, validators.EmailValidator):
                    ...
                    if "pattern" not in schema:
                        pattern = v.regex.pattern.encode(
                    ...
                    isinstance(v, validators.FileExtensionValidator)
                    and v.allowed_extensions
                ):
                    if "pattern" not in schema:
                        schema["pattern"] = "(?:%s)$" % "|".join(
                            [re.escape(extn) for extn in v.allowed_extensions]
                        )
    ...
    def map_parsers(self):
        return list(
            dict.fromkeys(
                [
                    p.media_type
                    for p in self.view.get_parsers()
    ...
    def _get_examples(
        ...
        for example in examples + extras:
            ...
            if not example.media_type:
                example_media_type = (
                    media_type if (example in extras) else "application/json"
                )
            else:
            ...
            if not example.status_codes:
                example_status_codes = (
                    (status_code,) if (example in extras) else ("200", "201")
                )
            else:
            ...
            if (
                ...
                example.value = build_listed_example_value(
                    example.value, self._get_paginator(), direction
                )
    ...
    def _get_request_for_media_type(self, serializer, direction="request"):
        ...
        if is_list_serializer(serializer):
            ...
            readonly_props = [
                p
                for p, s in component.schema.get("properties", {}).items()
                if s.get("readOnly")
            ]
            required_props = component.schema.get("required", [])
            request_body_required = any(
                req not in readonly_props for req in required_props
            )
    ...
    def _get_response_bodies(self, direction="response"):
        ...
        if (
            ...
            if self._is_create_operation():
                return {
                    "201": self._get_response_for_code(
                        response_serializers, "201", direction=direction
                    )
                }
            return {
                "200": self._get_response_for_code(
                    response_serializers, "200", direction=direction
                )
            }
            ...
            return {
                "200": self._get_response_for_code(schema, "200", direction=direction)
            }
    ...
    def _get_response_for_code(
        ...
        if (
            ...
            if is_list_serializer_customized(
                serializer
            ) and OpenApiSerializerExtension.get_match(get_list_serializer(serializer)):
                schema = self._map_serializer(
                    get_list_serializer(serializer), direction
                )
            else:
                schema = build_array_type(schema)
            ...
            **headers,
            "content": {
                media_type: build_media_type_object(
                    schema,
                    self._get_examples(
                        serializer, direction, media_type, status_code, examples
                    ),
                )
    ...
    def _get_serializer_name(self, serializer, direction, bypass_extensions=False):
        serializer_extension = OpenApiSerializerExtension.get_match(serializer)
        if serializer_extension and not bypass_extensions:
            custom_name = serializer_extension.get_name(
                **filter_supported_arguments(
